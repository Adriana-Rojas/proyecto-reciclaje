...
class BrigadesConfigurationBrigadeSponsor extends CI_Controller {
	...
	public function board() {
		...
		$mainPage = "BrigadesConfigurationBrigadeSponsor/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			// Lista de listas
			$data ['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla ( "BRI_CONVENIOBRIG" );
	...
	public function newRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$data ['title'] = "Crear convenio para brigadas";
			$data ['mainField'] = "Nombre";
			$data ['placeHolder'] = "Ej. Organizaci&oacute;n de estados americanos ";
			$data ['pagina'] = "BrigadesConfigurationBrigadeSponsor/saveRegister";
			$data ['mainPage'] = $mainPage;
			
			// Cargo vista
			$this->load->view ( 'common/forms/formOneValue', $data );
			// Cargo validación de formulario
			$this->load->view ( 'validation/brigades/configuration/BrigadesConfigurationBrigadeSponsorValidation' );
	...
	public function edit($id) {
		...
		$mainPage = "BrigadesConfigurationBrigadeSponsor/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			$id = $this->FunctionsGeneral->getFieldFromTable ( "BRI_CONVENIOBRIG", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				...
				$data ['nombre'] = $this->FunctionsGeneral->getFieldFromTable ( "BRI_CONVENIOBRIG", "NOMBRE", $id );
				// Inicializo variables de los campos del formulario
				$data ['title'] = "Modificar convenio para brigadas";
				$data ['mainField'] = "Nombre";
				$data ['placeHolder'] = "Ej. Organizaci&oacute;n de estados americanos ";
				$data ['pagina'] = "BrigadesConfigurationBrigadeSponsor/saveRegister";
				$data ['mainPage'] = $mainPage;
				...
				redirect ( base_url () . "BrigadesConfigurationBrigadeSponsor/board" );
			}
		} else {
			// Retorno a la página principal
			header ( "Location: " . base_url () );
	...
	public function saveRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			if ($this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'valida' ) ) ) == 'newRegister') {
				if ($this->FunctionsGeneral->getQuantityFieldFromTable ( "BRI_CONVENIOBRIG", "NOMBRE", $nombre ) == 0) {
					// Creo el registro
					$id = $this->FunctionsGeneral->insertOneParameter ( "BRI_CONVENIOBRIG", "NOMBRE", $nombre, $this->session->userdata ( 'usuario' ) );
				...
				$this->FunctionsGeneral->updateByID ( "BRI_CONVENIOBRIG", "NOMBRE", $nombre, $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
	...
	public function inactive($id) {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$id = $this->FunctionsGeneral->getFieldFromTable ( "BRI_CONVENIOBRIG", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				$estado = $this->FunctionsGeneral->getFieldFromTable ( "BRI_CONVENIOBRIG", "ESTADO", $id );
				if ($estado == 'S') {
					$estado = 'N';
				} else if ($estado == 'N') {
					$estado = 'S';
				...
				$this->FunctionsGeneral->updateByID ( "BRI_CONVENIOBRIG", "ESTADO", $estado, $id, $this->session->userdata ( 'usuario' ) );
				// Pinto mensaje para retornar a la aplicación
				$this->session->set_userdata ( 'id', $id );

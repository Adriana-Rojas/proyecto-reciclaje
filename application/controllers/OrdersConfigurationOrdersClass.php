...
class OrdersConfigurationOrdersOrdersConfigurationOrdersClass extends CI_Controller
{
    ...
    public function board()
    {
        ...
        $mainPage = "OrdersConfigurationOrdersClass/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            // Pinto las vistas adicionales a través de la función pintaComun del helper hospitium
            $mainPage = "OrdersConfigurationOrdersOrdersConfigurationOrdersClass/board";
            $data = null;
            ...
            // Lista de listas
            $data['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla("ORD_CLASETIPO");
    ...
    public function newRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $data['title'] = "Crear clase de orden ";
            $data['mainField'] = "Clase de orden";
            $data['placeHolder'] = "Ej.  &Aacute;rbol de elementos ";
            $data['pagina'] = "OrdersConfigurationOrdersOrdersConfigurationOrdersClass/saveRegister";
            $data['mainPage'] = $mainPage;
            
            // Cargo vista
            $this->load->view('common/forms/formOneValue', $data);
            // Cargo validación de formulario
            $this->load->view('validation/orders/configuration/ordersConfigurationOrdersOrdersConfigurationOrdersClassValidation');
    ...
    public function edit($id)
    {
        ...
        $mainPage = "OrdersConfigurationOrdersClass/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_CLASETIPO", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                ...
                $data['nombre'] = $this->FunctionsGeneral->getFieldFromTable("ORD_CLASETIPO", "NOMBRE", $id);
                // Inicializo variables de los campos del formulario
                $data['title'] = "Modificar clase de orden ";
                $data['mainField'] = "Clase de orden";
                $data['placeHolder'] = "Ej.  &Aacute;rbol de elementos ";
                $data['pagina'] = "OrdersConfigurationOrdersOrdersConfigurationOrdersClass/saveRegister";
                $data['mainPage'] = $mainPage;
                ...
                redirect(base_url() . "OrdersConfigurationOrdersOrdersConfigurationOrdersClass/board");
            }
        } else {
            // Retorno a la página principal
            header("Location: " . base_url());
    ...
    public function saveRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida'))) == 'newRegister') {
                if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_CLASETIPO", "NOMBRE", $nombre) == 0) {
                    // Creo el registro
                    $id = $this->FunctionsGeneral->insertOneParameter("ORD_CLASETIPO", "NOMBRE", $nombre, $this->session->userdata('usuario'));
                ...
                $this->FunctionsGeneral->updateByID("ORD_CLASETIPO", "NOMBRE", $nombre, $this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
    ...
    public function inactive($id)
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_CLASETIPO", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                $estado = $this->FunctionsGeneral->getFieldFromTable("ORD_CLASETIPO", "ESTADO", $id);
                if ($estado == 'S') {
                    $estado = 'N';
                } else if ($estado == 'N') {
                    $estado = 'S';
                ...
                $this->FunctionsGeneral->updateByID("ORD_CLASETIPO", "ESTADO", $estado, $id, $this->session->userdata('usuario'));
                // Pinto mensaje para retornar a la aplicación
                $this->session->set_userdata('id', $id);

 ...
 *************************************************************************
 ...
 Propósito:						Controlador para visualizar el manejo de las fases de las brigadas
 *************************************************************************
 *************************************************************************
 ******************** BOGOTÁ COLOMBIA 2017 *******************************
 */
defined ( 'BASEPATH' ) or exit ( 'No direct script access allowed' );
class BrigadesConfigurationBrigadeStage extends CI_Controller {
	...
	public function board() {
		...
		$mainPage = "BrigadesConfigurationBrigadeStage/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			// Lista de listas
			$data ['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla ( "BRI_FASEBRIG" );
	...
	public function newRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$data ['title'] = "Crear fase de brigadas";
			$data ['mainField'] = "Nombre";
			$data ['placeHolder'] = "Ej. Revi&oacute;n M&eacute;dica ";
			$data ['pagina'] = "BrigadesConfigurationBrigadeStage/saveRegister";
			$data ['mainPage'] = $mainPage;
			
			// Cargo vista
			$this->load->view ( 'common/forms/formOneValue', $data );
			// Cargo validación de formulario
			$this->load->view ( 'validation/brigades/configuration/BrigadesConfigurationBrigadeStageValidation' );
	...
	public function edit($id) {
		...
		$mainPage = "BrigadesConfigurationBrigadeStage/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			$id = $this->FunctionsGeneral->getFieldFromTable ( "BRI_FASEBRIG", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				...
				$data ['nombre'] = $this->FunctionsGeneral->getFieldFromTable ( "BRI_FASEBRIG", "NOMBRE", $id );
				// Inicializo variables de los campos del formulario
				$data ['title'] = "Modificar fase de brigadas";
				$data ['mainField'] = "Nombre";
				$data ['placeHolder'] = "Ej. Revi&oacute;n M&eacute;dica ";
				$data ['pagina'] = "BrigadesConfigurationBrigadeStage/saveRegister";
				$data ['mainPage'] = $mainPage;
				...
				redirect ( base_url () . "BrigadesConfigurationBrigadeStage/board" );
			}
		} else {
			// Retorno a la página principal
			header ( "Location: " . base_url () );
	...
	public function saveRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			if ($this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'valida' ) ) ) == 'newRegister') {
				if ($this->FunctionsGeneral->getQuantityFieldFromTable ( "BRI_FASEBRIG", "NOMBRE", $nombre ) == 0) {
					// Creo el registro
					$id = $this->FunctionsGeneral->insertOneParameter ( "BRI_FASEBRIG", "NOMBRE", $nombre, $this->session->userdata ( 'usuario' ) );
				...
				$this->FunctionsGeneral->updateByID ( "BRI_FASEBRIG", "NOMBRE", $nombre, $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
	...
	public function inactive($id) {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$id = $this->FunctionsGeneral->getFieldFromTable ( "BRI_FASEBRIG", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				$estado = $this->FunctionsGeneral->getFieldFromTable ( "BRI_FASEBRIG", "ESTADO", $id );
				if ($estado == 'S') {
					$estado = 'N';
				} else if ($estado == 'N') {
					$estado = 'S';
				...
				$this->FunctionsGeneral->updateByID ( "BRI_FASEBRIG", "ESTADO", $estado, $id, $this->session->userdata ( 'usuario' ) );
				// Pinto mensaje para retornar a la aplicación
				$this->session->set_userdata ( 'id', $id );

 ...
 *************************************************************************
 ...
 Propósito:						Controlador para visualizar el manejo de las partes del cuerpo que aplican dentro del proceso de la aplicación de órdenes.
 *************************************************************************
 *************************************************************************
...
class OrdersConfigurationElementCharacteristics extends CI_Controller
{
    ...
    public function board()
    {
        ...
        $mainPage = "OrdersConfigurationElementCharacteristics/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            // Lista de listas
            $data['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla("ORD_PARELEM");
    ...
    public function newRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $data['title'] = "Crear caracter&iacute;stica para un elemento";
            $data['mainField'] = "Caracter&iacute;stica";
            $data['placeHolder'] = "Ej. Talla ";
            $data['pagina'] = "OrdersConfigurationElementCharacteristics/saveRegister";
            $data['mainPage'] = $mainPage;
            
            // Cargo vista
            $this->load->view('common/forms/formOneValue', $data);
            // Cargo validación de formulario
            $this->load->view('validation/orders/configuration/ordersConfigurationElementCharacteristicsValidation');
    ...
    public function edit($id)
    {
        ...
        $mainPage = "OrdersConfigurationElementCharacteristics/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_PARELEM", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                ...
                $data['nombre'] = $this->FunctionsGeneral->getFieldFromTable("ORD_PARELEM", "NOMBRE", $id);
                // Inicializo variables de los campos del formulario
                $data['title'] = "Modificar caracter&iacute;stica para un elemento previamente creado";
                $data['mainField'] = "Caracter&iacute;stica";
                $data['placeHolder'] = "Ej. Talla ";
                $data['pagina'] = "OrdersConfigurationElementCharacteristics/saveRegister";
                $data['mainPage'] = $mainPage;
                ...
                redirect(base_url() . "OrdersConfigurationElementCharacteristics/board");
            }
        } else {
            // Retorno a la página principal
            header("Location: " . base_url());
    ...
    public function saveRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida'))) == 'newRegister') {
                if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_PARELEM", "NOMBRE", $nombre) == 0) {
                    // Creo el registro
                    $id = $this->FunctionsGeneral->insertOneParameter("ORD_PARELEM", "NOMBRE", $nombre, $this->session->userdata('usuario'));
                ...
                $this->FunctionsGeneral->updateByID("ORD_PARELEM", "NOMBRE", $nombre, $this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
    ...
    public function inactive($id)
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_PARELEM", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                $estado = $this->FunctionsGeneral->getFieldFromTable("ORD_PARELEM", "ESTADO", $id);
                if ($estado == 'S') {
                    $estado = 'N';
                } else if ($estado == 'N') {
                    $estado = 'S';
                ...
                $this->FunctionsGeneral->updateByID("ORD_PARELEM", "ESTADO", $estado, $id, $this->session->userdata('usuario'));
                // Pinto mensaje para retornar a la aplicación
                $this->session->set_userdata('id', $id);

...
class ShelterConfigurationDefineBed extends CI_Controller {
 ...
 public function __construct()
	...
	public function board(){
        ...
        $mainPage="ShelterConfigurationDefineBed/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
            ...
            //Lista de listas
            $data['listaLista']=$this->FunctionsGeneral->selectValoresListaTabla("HP_CAMAS");
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$data['title']="Crear definici&oacute;n de camas ";
    		$data['mainField']="Nombre";
    		$data['placeHolder']="Ej.  Cama A ";
    		$data['pagina']="ShelterConfigurationDefineBed/saveRegister";
    		$data['mainPage']=$mainPage;
    		
    		//Cargo vista
    		$this->load->view('common/forms/formOneValue',$data);
    		// Cargo validación de formulario
    		$this->load->view('validation/shelter/configuration/shelterConfigurationDefineBedValidation');
    ...
    public function edit($id){
    	...
    	$mainPage="ShelterConfigurationDefineBed/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		$id=$this->FunctionsGeneral->getFieldFromTable("HP_CAMAS","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			...
    			$data['nombre']=$this->FunctionsGeneral->getFieldFromTable("HP_CAMAS","NOMBRE",$id);
    			//Inicializo variables de los campos del formulario
    			$data['title']="Modificar definici&oacute;n de camas ";
	    		$data['mainField']="Nombre";
	    		$data['placeHolder']="Ej.  Cama A ";
    			$data['pagina']="ShelterConfigurationDefineBed/saveRegister";
    			$data['mainPage']=$mainPage;
    			...
    			redirect(base_url()."ShelterConfigurationDefineBed/board");
    		}
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida')))=='newRegister'){
    			if ($this->FunctionsGeneral->getQuantityFieldFromTable("HP_CAMAS","NOMBRE",$nombre)==0){
    				//Creo el registro
    				$id=$this->FunctionsGeneral->insertOneParameter("HP_CAMAS","NOMBRE",$nombre, $this->session->userdata('usuario'));
    			...
    			$this->FunctionsGeneral->updateByID("HP_CAMAS","NOMBRE", 
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$id=$this->FunctionsGeneral->getFieldFromTable("HP_CAMAS","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			$estado=$this->FunctionsGeneral->getFieldFromTable("HP_CAMAS","ESTADO",$id);
    			if($estado=='S'){
    				$estado='N';

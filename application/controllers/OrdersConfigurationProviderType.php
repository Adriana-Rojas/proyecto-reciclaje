 ...
 *************************************************************************
 ...
 Propósito:						Controlador para visualizar el manejo de los tipos de proveedores dentro de la aplicación de órdenes.
 *************************************************************************
 *************************************************************************
...
class OrdersConfigurationProviderType extends CI_Controller
{
    ...
    public function board()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            // Lista de listas
            $data['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla("ORD_TIPOPROV");
    ...
    public function newRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $data['title'] = "Crear tipo de proveedor ";
            $data['mainField'] = "Tipo de proveedor";
            $data['placeHolder'] = "Ej. M&uacute;ltiple ";
            $data['pagina'] = "OrdersConfigurationProviderType/saveRegister";
            $data['mainPage'] = $mainPage;
            ...
            $this->load->view('validation/orders/configuration/ordersConfigurationProviderTypeValidation');
    ...
    public function edit($id)
    {
        ...
        $mainPage = "OrdersConfigurationProviderType/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_TIPOPROV", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                ...
                $data['nombre'] = $this->FunctionsGeneral->getFieldFromTable("ORD_TIPOPROV", "NOMBRE", $id);
                // Inicializo variables de los campos del formulario
                $data['title'] = "Modificar tipo de proveedor";
                $data['mainField'] = "Tipo de proveedor";
                $data['placeHolder'] = "Ej. M&uacute;ltiple ";
                $data['pagina'] = "OrdersConfigurationProviderType/saveRegister";
                $data['mainPage'] = $mainPage;
    ...
    public function saveRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
			...
			if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida'))) == 'newRegister') {
                if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_TIPOPROV", "NOMBRE", $nombre) == 0) {
                    // Creo el registro
                    $id = $this->FunctionsGeneral->insertOneParameter("ORD_TIPOPROV", "NOMBRE", $nombre, $this->session->userdata('usuario'));
                ...
                $this->FunctionsGeneral->updateByID("ORD_TIPOPROV", "NOMBRE", $nombre, $this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
    ...
    public function inactive($id)
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $id = $this->FunctionsGeneral->getFieldFromTable("ORD_TIPOPROV", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                $estado = $this->FunctionsGeneral->getFieldFromTable("ORD_TIPOPROV", "ESTADO", $id);
                if ($estado == 'S') {
                    $estado = 'N';
                } else if ($estado == 'N') {
                    $estado = 'S';
                ...
                $this->FunctionsGeneral->updateByID("ORD_TIPOPROV", "ESTADO", $estado, $id, $this->session->userdata('usuario'));
                // Pinto mensaje para retornar a la aplicación
                $this->session->set_userdata('id', $id);

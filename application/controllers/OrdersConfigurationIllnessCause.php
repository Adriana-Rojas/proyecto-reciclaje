...
class OrdersConfigurationIllnessCause extends CI_Controller {
	...
	public function board() {
		...
		$mainPage = "OrdersConfigurationIllnessCause/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			// Lista de listas
			$data ['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla ( "ORD_CAUSAENFERMEDAD" );
	...
	public function newRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$data ['title'] = "Crear causas de enfermedad";
			$data ['mainField'] = "Causa de enfermedad";
			$data ['placeHolder'] = "Ej. Accidente de trabajo ";
			$data ['pagina'] = "OrdersConfigurationIllnessCause/saveRegister";
			$data ['mainPage'] = $mainPage;
			
			// Cargo vista
			$this->load->view ( 'common/forms/formOneValue', $data );
			// Cargo validación de formulario
			$this->load->view ( 'validation/orders/configuration/OrdersConfigurationIllnessCauseValidation' );
	...
	public function edit($id) {
		...
		$mainPage = "OrdersConfigurationIllnessCause/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			$id = $this->FunctionsGeneral->getFieldFromTable ( "ORD_CAUSAENFERMEDAD", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				...
				$data ['nombre'] = $this->FunctionsGeneral->getFieldFromTable ( "ORD_CAUSAENFERMEDAD", "NOMBRE", $id );
				// Inicializo variables de los campos del formulario
				$data ['title'] = "Modificar  causas de enfermedad";
				$data ['mainField'] = "Causa de enfermedad";
				$data ['placeHolder'] = "Ej. Accidente de trabajo ";
				$data ['pagina'] = "OrdersConfigurationIllnessCause/saveRegister";
				$data ['mainPage'] = $mainPage;
				...
				redirect ( base_url () . "OrdersConfigurationIllnessCause/board" );
			}
		} else {
			// Retorno a la página principal
			header ( "Location: " . base_url () );
	...
	public function saveRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			if ($this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'valida' ) ) ) == 'newRegister') {
				if ($this->FunctionsGeneral->getQuantityFieldFromTable ( "ORD_CAUSAENFERMEDAD", "NOMBRE", $nombre ) == 0) {
					// Creo el registro
					$id = $this->FunctionsGeneral->insertOneParameter ( "ORD_CAUSAENFERMEDAD", "NOMBRE", $nombre, $this->session->userdata ( 'usuario' ) );
				...
				$this->FunctionsGeneral->updateByID ( "ORD_CAUSAENFERMEDAD", "NOMBRE", $nombre, $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
	...
	public function inactive($id) {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$id = $this->FunctionsGeneral->getFieldFromTable ( "ORD_CAUSAENFERMEDAD", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				$estado = $this->FunctionsGeneral->getFieldFromTable ( "ORD_CAUSAENFERMEDAD", "ESTADO", $id );
				if ($estado == 'S') {
					$estado = 'N';
				} else if ($estado == 'N') {
					$estado = 'S';
				...
				$this->FunctionsGeneral->updateByID ( "ORD_CAUSAENFERMEDAD", "ESTADO", $estado, $id, $this->session->userdata ( 'usuario' ) );
				// Pinto mensaje para retornar a la aplicación
				$this->session->set_userdata ( 'id', $id );

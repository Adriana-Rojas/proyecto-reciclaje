...
class SponsorshipsDefineType extends CI_Controller {
 ...
 public function __construct()
	...
	public function board(){
        ...
        $mainPage="SponsorshipsDefineType/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
            ...
            //Lista de listas
            $data['listaLista']=$this->FunctionsGeneral->selectValoresListaTabla("PAT_TIPO");
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$data['title']="Crear tipos de patrocinios";
    		$data['mainField']="Tipo";
    		$data['placeHolder']="Ej. Patrocinios especiales ";
    		$data['pagina']="SponsorshipsDefineType/saveRegister";
    		$data['mainPage']=$mainPage;
    		
    		//Cargo vista
    		$this->load->view('common/forms/formOneValue',$data);
    		// Cargo validación de formulario
    		$this->load->view('validation/sponsorships/configuration/sponsorshipsDefineTypeValidation');
    ...
    public function edit($id){
    	...
    	$mainPage="SponsorshipsDefineType/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		$id=$this->FunctionsGeneral->getFieldFromTable("PAT_TIPO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			...
    			$data['nombre']=$this->FunctionsGeneral->getFieldFromTable("PAT_TIPO","NOMBRE",$id);
    			//Inicializo variables de los campos del formulario
    			$data['title']="Modificar tipos de patrocinios";
	    		$data['mainField']="Tipo";
	    		$data['placeHolder']="Ej. Patrocinios especiales ";
	    		$data['pagina']="SponsorshipsDefineType/saveRegister";
	    		$data['pagina']="SponsorshipsDefineType/saveRegister";
    			...
    			redirect(base_url()."SponsorshipsDefineType/board");
    		}
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida')))=='newRegister'){
    			if ($this->FunctionsGeneral->getQuantityFieldFromTable("PAT_TIPO","NOMBRE",$nombre)==0){
    				//Creo el registro
    				$id=$this->FunctionsGeneral->insertOneParameter("PAT_TIPO","NOMBRE",$nombre, $this->session->userdata('usuario'));
    			...
    			$this->FunctionsGeneral->updateByID("PAT_TIPO","NOMBRE", 
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$id=$this->FunctionsGeneral->getFieldFromTable("PAT_TIPO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			$estado=$this->FunctionsGeneral->getFieldFromTable("PAT_TIPO","ESTADO",$id);
    			if($estado=='S'){
    				$estado='N';

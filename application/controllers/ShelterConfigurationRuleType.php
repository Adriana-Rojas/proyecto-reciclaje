...
class ShelterConfigurationRuleType extends CI_Controller
{
    ...
    public function board()
    {
        ...
        $mainPage = "ShelterConfigurationRuleType/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            // Lista de listas
            $data['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla("HP_TIPOREGLA");
    ...
    public function newRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $data['title'] = "Crear definici&oacute;n de tipos de reglas ";
            $data['mainField'] = "Nombre";
            $data['placeHolder'] = "Ej.  Consecutivo reserva ";
            $data['pagina'] = "ShelterConfigurationRuleType/saveRegister";
            $data['mainPage'] = $mainPage;
            
            // Cargo vista
            $this->load->view('common/forms/formOneValue', $data);
            // Cargo validación de formulario
            $this->load->view('validation/shelter/configuration/shelterConfigurationRuleTypeValidation');
    ...
    public function edit($id)
    {
        ...
        $mainPage = "ShelterConfigurationRuleType/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            $id = $this->FunctionsGeneral->getFieldFromTable("HP_TIPOREGLA", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                ...
                $data['nombre'] = $this->FunctionsGeneral->getFieldFromTable("HP_TIPOREGLA", "NOMBRE", $id);
                // Inicializo variables de los campos del formulario
                $data['title'] = "Modificar definici&oacute;n de tipos de reglas ";
                $data['mainField'] = "Nombre";
                $data['placeHolder'] = "Ej.  Consecutivo reserva";
                $data['pagina'] = "ShelterConfigurationRuleType/saveRegister";
                $data['mainPage'] = $mainPage;
                ...
                redirect(base_url() . "ShelterConfigurationRuleType/board");
            }
        } else {
            // Retorno a la página principal
            header("Location: " . base_url());
    ...
    public function saveRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida'))) == 'newRegister') {
                if ($this->FunctionsGeneral->getQuantityFieldFromTable("HP_TIPOREGLA", "NOMBRE", $nombre) == 0) {
                    // Creo el registro
                    $id = $this->FunctionsGeneral->insertOneParameter("HP_TIPOREGLA", "NOMBRE", $nombre, $this->session->userdata('usuario'));
                ...
                $this->FunctionsGeneral->updateByID("HP_TIPOREGLA", "NOMBRE", $nombre, $this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
    ...
    public function inactive($id)
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $id = $this->FunctionsGeneral->getFieldFromTable("HP_TIPOREGLA", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                $estado = $this->FunctionsGeneral->getFieldFromTable("HP_TIPOREGLA", "ESTADO", $id);
                if ($estado == 'S') {
                    $estado = 'N';
                } else if ($estado == 'N') {
                    $estado = 'S';
                ...
                $this->FunctionsGeneral->updateByID("HP_TIPOREGLA", "ESTADO", $estado, $id, $this->session->userdata('usuario'));
                // Pinto mensaje para retornar a la aplicación
                $this->session->set_userdata('id', $id);

...
class OrdersConfigurationProcess extends CI_Controller {
	...
	public function board() {
		...
		$mainPage = "OrdersConfigurationProcess/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			// Lista de listas
			$data ['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla ( "ORD_PROCESO" );
	...
	public function newRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$data ['title'] = "Crear tipo de proceso de atenci&oacute;n ";
			$data ['mainField'] = "Proceso";
			$data ['placeHolder'] = "Ej.  Normal (interno) ";
			$data ['pagina'] = "OrdersConfigurationProcess/saveRegister";
			$data ['mainPage'] = $mainPage;
			$data ['mascara'] = false;
			$data ['apoyoField'] = "&Iacute;cono";
			$data ['placeHolderApoyo'] = "Ej.  fa fa-users ";
			
			// Cargo vista
			$this->load->view ( 'common/forms/formTwoValues', $data );
			// Cargo validación de formulario
			$this->load->view ( 'validation/orders/configuration/ordersConfigurationProcessValidation' );
	...
	public function edit($id) {
		...
		$mainPage = "OrdersConfigurationProcess/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			$id = $this->FunctionsGeneral->getFieldFromTable ( "ORD_PROCESO", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				...
				$data ['nombre'] = $this->FunctionsGeneral->getFieldFromTable ( "ORD_PROCESO", "NOMBRE", $id );
				$data ['apoyo'] = $this->FunctionsGeneral->getFieldFromTable ( "ORD_PROCESO", "ICONO", $id );;
				// Inicializo variables de los campos del formulario
				$data ['title'] = "Modificar tipo de proceso de atenci&oacute;n ";
				$data ['mainField'] = "Proceso";
				$data ['placeHolder'] = "Ej.  Normal (interno) ";
				$data ['pagina'] = "OrdersConfigurationProcess/saveRegister";
				$data ['mainPage'] = $mainPage;
				...
				redirect ( base_url () . "OrdersConfigurationProcess/board" );
			}
		} else {
			// Retorno a la página principal
			header ( "Location: " . base_url () );
	...
	public function saveRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			if ($this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'valida' ) ) ) == 'newRegister') {
				if ($this->FunctionsGeneral->getQuantityFieldFromTable ( "ORD_PROCESO", "NOMBRE", $nombre ) == 0) {
					// Creo el registro
				    $id = $this->FunctionsGeneral->insertOneParameter ( "ORD_PROCESO", "NOMBRE", $nombre,"ICONO", $this->security->xss_clean ( $this->input->post ( 'apoyo' ) ), $this->session->userdata ( 'usuario' ) );
				...
				$this->FunctionsGeneral->updateByID ( "ORD_PROCESO", "NOMBRE", $nombre, $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
				$this->FunctionsGeneral->updateByID ( "ORD_PROCESO", "ICONO", $this->security->xss_clean ( $this->input->post ( 'apoyo' ) ), $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
	...
	public function inactive($id) {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$id = $this->FunctionsGeneral->getFieldFromTable ( "ORD_PROCESO", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				$estado = $this->FunctionsGeneral->getFieldFromTable ( "ORD_PROCESO", "ESTADO", $id );
				if ($estado == 'S') {
					$estado = 'N';
				} else if ($estado == 'N') {
					$estado = 'S';
				...
				$this->FunctionsGeneral->updateByID ( "ORD_PROCESO", "ESTADO", $estado, $id, $this->session->userdata ( 'usuario' ) );
				// Pinto mensaje para retornar a la aplicación
				$this->session->set_userdata ( 'id', $id );

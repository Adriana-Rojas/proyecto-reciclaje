...
class SponsorshipsDefineFund extends CI_Controller {
	...
	public function board() {
		...
		$mainPage = "SponsorshipsDefineFund/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			// Lista de listas
			$data ['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla ( "PAT_FONDOS" );
	...
	public function newRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$data ['title'] = "Crear fondos para patrocinios";
			$data ['mainField'] = "Fondo";
			$data ['placeHolder'] = "Ej. La casita ";
			$data ['pagina'] = "SponsorshipsDefineFund/saveRegister";
			$data ['mainPage'] = $mainPage;
			
			// Cargo vista
			$this->load->view ( 'common/forms/formOneValue', $data );
			// Cargo validación de formulario
			$this->load->view ( 'validation/sponsorships/configuration/SponsorshipsDefineFundValidation' );
	...
	public function edit($id) {
		...
		$mainPage = "SponsorshipsDefineFund/board";
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			$id = $this->FunctionsGeneral->getFieldFromTable ( "PAT_FONDOS", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				...
				$data ['nombre'] = $this->FunctionsGeneral->getFieldFromTable ( "PAT_FONDOS", "NOMBRE", $id );
				// Inicializo variables de los campos del formulario
				$data ['title'] = "Modificar fondos para patrocinios";
				$data ['mainField'] = "Fondo";
				$data ['placeHolder'] = "Ej. La casita ";
				$data ['pagina'] = "SponsorshipsDefineFund/saveRegister";
				$data ['mainPage'] = $mainPage;
				...
				redirect ( base_url () . "SponsorshipsDefineFund/board" );
			}
		} else {
			// Retorno a la página principal
			header ( "Location: " . base_url () );
	...
	public function saveRegister() {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			if ($this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'valida' ) ) ) == 'newRegister') {
				if ($this->FunctionsGeneral->getQuantityFieldFromTable ( "PAT_FONDOS", "NOMBRE", $nombre ) == 0) {
					// Creo el registro
					$idFondo = $this->FunctionsGeneral->insertOneParameter ( "PAT_FONDOS", "NOMBRE", $nombre, $this->session->userdata ( 'usuario' ) );
					//Creo en ceros el saldo del patrocinio para el mes actual
					$this->SponsorshipModel->insertBalanceFund(
							$idFondo, 
							date('m'), 
							date('Y'),
							0, 
				...
				$this->FunctionsGeneral->updateByID ( "PAT_FONDOS", "NOMBRE", $nombre, $this->encryption->decrypt ( $this->security->xss_clean ( $this->input->post ( 'id' ) ) ), $this->session->userdata ( 'usuario' ) );
	...
	public function inactive($id) {
		...
		if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
			...
			$id = $this->FunctionsGeneral->getFieldFromTable ( "PAT_FONDOS", "ID", $this->encryption->decrypt ( $id ) );
			if ($id != '') {
				$estado = $this->FunctionsGeneral->getFieldFromTable ( "PAT_FONDOS", "ESTADO", $id );
				if ($estado == 'S') {
					$estado = 'N';
				} else if ($estado == 'N') {
					$estado = 'S';
				...
				$this->FunctionsGeneral->updateByID ( "PAT_FONDOS", "ESTADO", $estado, $id, $this->session->userdata ( 'usuario' ) );
				// Pinto mensaje para retornar a la aplicación
				$this->session->set_userdata ( 'id', $id );

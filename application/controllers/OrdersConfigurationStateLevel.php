...
class OrdersConfigurationStateLevel extends CI_Controller {
 ...
 public function __construct()
	...
	public function board(){
        ...
        $mainPage="OrdersConfigurationStateLevel/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
            ...
            //Lista de listas
            $data['listaLista']=$this->FunctionsGeneral->selectValoresListaTabla("ORD_NIVELESTADO");
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$data['title']="Crear nivel de estados ";
    		$data['mainField']="Nivel";
	    	$data['placeHolder']="Ej.  Primario ";
    		$data['pagina']="OrdersConfigurationStateLevel/saveRegister";
    		$data['mainPage']=$mainPage;
    		
    		//Cargo vista
    		$this->load->view('common/forms/formOneValue',$data);
    		// Cargo validación de formulario
    		$this->load->view('validation/orders/configuration/ordersConfigurationStateLevelValidation');
    ...
    public function edit($id){
    	...
    	$mainPage="OrdersConfigurationStateLevel/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVELESTADO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			...
    			$data['nombre']=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVELESTADO","NOMBRE",$id);
    			//Inicializo variables de los campos del formulario
    			$data['title']="Modificar nivel de estados ";
	    		$data['mainField']="Nivel";
		    	$data['placeHolder']="Ej.  Primario ";
    			$data['pagina']="OrdersConfigurationStateLevel/saveRegister";
    			$data['mainPage']=$mainPage;
    			...
    			redirect(base_url()."OrdersConfigurationStateLevel/board");
    		}
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida')))=='newRegister'){
    			if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_NIVELESTADO","NOMBRE",$nombre)==0){
    				//Creo el registro
    				$id=$this->FunctionsGeneral->insertOneParameter("ORD_NIVELESTADO","NOMBRE",$nombre, $this->session->userdata('usuario'));
    			...
    			$this->FunctionsGeneral->updateByID("ORD_NIVELESTADO","NOMBRE", 
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVELESTADO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			$estado=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVELESTADO","ESTADO",$id);
    			if($estado=='S'){
    				$estado='N';

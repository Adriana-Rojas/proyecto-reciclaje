...
class ShelterConfigurationDefineStates extends CI_Controller
{
    ...
    public function board()
    {
        ...
        $mainPage = "ShelterConfigurationDefineStates/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            // Lista de listas
            $data['listaLista'] = $this->FunctionsGeneral->selectValoresListaTabla("HP_ESTADO");
    ...
    public function newRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            // Cargo vista
            $this->load->view('shelter/configuration/formStatesShelter', $data);
            // Cargo validación de formulario
            $this->load->view('validation/shelter/configuration/shelterConfigurationDefineStatesValidation');
    ...
    public function edit($id)
    {
        ...
        $mainPage = "ShelterConfigurationDefineStates/board";
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            $id = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                ...
                $data['nombre'] = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "NOMBRE", $id);
                $data['icono'] = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "ICONO", $id);
                ;
                
                // Lista de aplica
                $data['listaSiNo'] = $this->FunctionsAdmin->selectValoresListaAdministracion('SI_NO', '1');
                $data['incluye'] = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "PERMITE", $id);
                ;
                ...
                redirect(base_url() . "ShelterConfigurationDefineStates/board");
            }
        } else {
            // Retorno a la página principal
            header("Location: " . base_url());
    ...
    public function saveRegister()
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida'))) == 'newRegister') {
                if ($this->FunctionsGeneral->getQuantityFieldFromTable("HP_ESTADO", "NOMBRE", $nombre) == 0) {
                    // Creo el registro
                    $id = $this->ShelterModel->insertShelterState($nombre, $this->security->xss_clean($this->input->post('permite')), $this->security->xss_clean($this->input->post('icono')), $this->session->userdata('usuario'));
                ...
                $this->ShelterModel->updateShelterState($this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $nombre, $this->security->xss_clean($this->input->post('permite')), $this->security->xss_clean($this->input->post('icono')), $this->session->userdata('usuario'));
    ...
    public function inactive($id)
    {
        ...
        if ($this->FunctionsAdmin->validateSession($mainPage)) {
            ...
            $id = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "ID", $this->encryption->decrypt($id));
            if ($id != '') {
                $estado = $this->FunctionsGeneral->getFieldFromTable("HP_ESTADO", "ESTADO", $id);
                if ($estado == 'S') {
                    $estado = 'N';
                } else if ($estado == 'N') {
                    $estado = 'S';
                ...
                $this->FunctionsGeneral->updateByID("HP_ESTADO", "ESTADO", $estado, $id, $this->session->userdata('usuario'));
                // Pinto mensaje para retornar a la aplicación
                $this->session->set_userdata('id', $id);

 ...
 *************************************************************************
 ...
 Propósito:						Controlador para visualizar el manejo de níveles de los servicios médicos dentro de la aplicación de órdenes.
 *************************************************************************
 *************************************************************************
...
class OrdersConfigurationServicesLevels extends CI_Controller {
 ...
 public function __construct()
	...
	public function board(){
        ...
        $mainPage="OrdersConfigurationServicesLevels/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
            ...
            //Lista de listas
            $data['listaLista']=$this->FunctionsGeneral->selectValoresListaTabla("ORD_NIVSERVICIO");
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$data['title']="Crear tipos de servicios m&eacute;dicos";
    		$data['mainField']="Nombre";
    		$data['placeHolder']="Ej. Valoraciones ";
    		$data['father']="Tiempo de complejidad ";
    		$data['pagina']="OrdersConfigurationServicesLevels/saveRegister";
    		$data['mainPage']=$mainPage;
    		
    		//Cargo vista
    		$this->load->view('common/forms/formOneValueWithFather',$data);
    		// Cargo validación de formulario
    		$this->load->view('validation/orders/configuration/ordersConfigurationServicesLevels');
    ...
    public function edit($id){
    	...
    	$mainPage="OrdersConfigurationServicesLevels/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVSERVICIO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			...
    			$data['nombre']=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVSERVICIO","NOMBRE",$id);
    			$data['tipo']=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVSERVICIO","ID_TIEMPO",$id);
    			$data['listaPadre']=$this->FunctionsGeneral->selectValoresListaTabla("ORD_TIEMPO",'DESC');
    			
    			//Inicializo variables de los campos del formulario
    			$data['title']="Modificar tipos de servicios m&eacute;dicos";
	    		$data['mainField']="Nombre";
	    		$data['placeHolder']="Ej. Valoraciones ";
	    		$data['father']="Tiempo de complejidad ";
	    		$data['pagina']="OrdersConfigurationServicesLevels/saveRegister";
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida')))=='newRegister'){
    			if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_NIVSERVICIO","NOMBRE",$nombre)==0){
    				//Creo el registro
    				$id=$this->FunctionsGeneral->insertTwoParameter("ORD_NIVSERVICIO","NOMBRE",$nombre,"ID_TIEMPO",$this->security->xss_clean($this->input->post('padre')), $this->session->userdata('usuario'));
    			...
    			$this->FunctionsGeneral->updateByID("ORD_NIVSERVICIO","NOMBRE",
    					$nombre, $this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
    			$this->FunctionsGeneral->updateByID("ORD_NIVSERVICIO","ID_TIEMPO",
    					$this->security->xss_clean($this->input->post('padre')), 
    					$this->encryption->decrypt($this->security->xss_clean($this->input->post('id'))), $this->session->userdata('usuario'));
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVSERVICIO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			$estado=$this->FunctionsGeneral->getFieldFromTable("ORD_NIVSERVICIO","ESTADO",$id);
    			if($estado=='S'){
    				$estado='N';

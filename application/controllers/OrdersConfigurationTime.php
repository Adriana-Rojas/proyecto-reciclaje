...
class OrdersConfigurationTime extends CI_Controller {
 ...
 public function __construct()
	...
	public function board(){
        ...
        $mainPage="OrdersConfigurationTime/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
            ...
            //Lista de listas
            $data['listaLista']=$this->FunctionsGeneral->selectValoresListaTabla("ORD_TIEMPO");
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$data['title']="Crear variable para identificar tiempos de complejidad de elementos o productos previamente creado";
    		$data['mainField']="Valor";
    		$data['placeHolder']="Ej. Alto ";
    		$data['pagina']="OrdersConfigurationTime/saveRegister";
    		$data['mainPage']=$mainPage;
    		
    		//Cargo vista
    		$this->load->view('common/forms/formOneValue',$data);
    		// Cargo validación de formulario
    		$this->load->view('validation/orders/configuration/ordersConfigurationTimeValidation');
    ...
    public function edit($id){
    	...
    	$mainPage="OrdersConfigurationTime/board"; 
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_TIEMPO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			...
    			$data['nombre']=$this->FunctionsGeneral->getFieldFromTable("ORD_TIEMPO","NOMBRE",$id);
    			//Inicializo variables de los campos del formulario
    			$data['title']="Modificar variable para identificar tiempos de complejidad de elementos o productos previamente creado";
	    		$data['mainField']="Valor";
	    		$data['placeHolder']="Ej. Alto ";
	    		$data['pagina']="OrdersConfigurationTime/saveRegister";
    			$data['mainPage']=$mainPage;
    			...
    			redirect(base_url()."OrdersConfigurationTime/board");
    		}
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		if ($this->encryption->decrypt($this->security->xss_clean($this->input->post('valida')))=='newRegister'){
    			if ($this->FunctionsGeneral->getQuantityFieldFromTable("ORD_TIEMPO","NOMBRE",$nombre)==0){
    				//Creo el registro
    				$id=$this->FunctionsGeneral->insertOneParameter("ORD_TIEMPO","NOMBRE",$nombre, $this->session->userdata('usuario'));
    			...
    			$this->FunctionsGeneral->updateByID("ORD_TIEMPO","NOMBRE", 
...
if ($this->FunctionsAdmin->validateSession ( $mainPage )) {
    		...
    		$id=$this->FunctionsGeneral->getFieldFromTable("ORD_TIEMPO","ID",$this->encryption->decrypt($id));
    		if ($id!=''){
    			$estado=$this->FunctionsGeneral->getFieldFromTable("ORD_TIEMPO","ESTADO",$id);
    			if($estado=='S'){
    				$estado='N';
